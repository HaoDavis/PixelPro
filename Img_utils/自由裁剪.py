...
def on_mouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:  # 左键点击
        ...
        if cut_img is not None:
            cv2.imwrite('crop_cell_nucleus.tif', cut_img)  # crop_cell_nucleus.tif为裁剪后的图片
        else:
            print("Error: Cut image is empty")


def main():
    global img
    img = cv2.imread('pictrue.tif')  # cell_nucleus.tif为要裁剪的图片名
